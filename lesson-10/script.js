function Car () {
    this.fuelConsumption = arguments[0];
    this.engineType = arguments[1];
    this.engineVolume = arguments[2];
    this.modelName = arguments[3];
    this.madeYear = arguments[4];
    this.curbWeight = arguments[5];
...
const dealerPriceList = {
    hatchback: {
        ...
        weightCoef: {
            lightWeight: 1.5, // 800-1100
            mediumWeight: 1.7, // 1101-1400
            heavyWeight: 2, // >1401
        },
    },
    sedan: {
        basicRate: 125,
        yearCoef: {
            veryNew: 1.5, // 2019-2022
            new: 1.8, // 2010-2018
            old: 2, // 2000-2009
            veryOld: 2.6, // 1990-1999
        },
        engineCoef: {
            diesel: 2.5,
            gas: 2,
        ...
        weightCoef: {
            lightWeight: 1.6, // 800-1100
            mediumWeight: 1.8, // 1101-1400
            heavyWeight: 2.1, // >1401
        },
    },
    wagon: {
        basicRate: 130,
        ...
        weightCoef: {
            lightWeight: 1.7, // 800-1100
            mediumWeight: 1.9, // 1101-1400
            heavyWeight: 2.2, // >1401
        },
    }
}
...
const dealer = (car) => {
    ...
    if (car.carCondition >= 100) {
        return 'Car is OK. No need to maintenance. But you can buy accessories or order interior cleaning.'
    } 
    if (car.carCondition === 0) {
        return 'Car is fully broken. Maintenance is impossible.'
    }

    const carYearCoef = getYearCoef(dealerPrices.yearCoef, car.madeYear);

    if (!carYearCoef) {
        return `We don't support car ${car.madeYear} year maded.`
    } 
        
    const carEngineCoef = getEngineCoef(dealerPrices.engineCoef, car.engineType);

    if (!carEngineCoef) {
        return 'We specialized only on gas or diesel fuel engines.'
    }      
    
    const carWeightCoef = getWeightCoef(dealerPrices.weightCoef, car.curbWeight);

    if (!carWeightCoef) {
        return 'Sory, but we dont work with toy cars.'
    } 
